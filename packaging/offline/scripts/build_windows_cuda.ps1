...
$PREREQ_DIR = Join-Path $DIST_DIR "prerequisites"
...
# 固定版本，避免每次构建都重新下载；否则从微软官方地址获取。
Write-Host "[构建] 准备 VC++ 运行库..." -ForegroundColor Yellow
$VC_REDIST = $env:LELAB_VC_REDIST
if (-not ($VC_REDIST -and (Test-Path -LiteralPath $VC_REDIST))) {
    $VC_REDIST = Join-Path $BUILD_DIR "VC_redist.x64.exe"
    Invoke-WebRequest -Uri "https://aka.ms/vs/17/release/vc_redist.x64.exe" -OutFile $VC_REDIST
}
$VC_SIGNATURE = Get-AuthenticodeSignature -FilePath $VC_REDIST
if ($VC_SIGNATURE.Status -ne "Valid" -or $VC_SIGNATURE.SignerCertificate.Subject -notmatch "Microsoft") {
    throw "VC_redist.x64.exe 数字签名无效或不是 Microsoft 发布文件"
}
